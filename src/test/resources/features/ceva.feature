...
Feature: Login
  ...
  @MRA-403
    Scenario: User field must have
    Given I go to "http://localhost:3000/users"
    And I check for Profile photo and Full Name

  @MRA-404
    Scenario: Email field must have display the full email address
    Given I go to "http://localhost:3000/users"
    And I check for full email address

  @MRA-405+406
    Scenario: Checking status field colors
    Given I go to "http://localhost:3000/users"
  ...
  @MRA-407+408
    Scenario: Clicking on View button and expected behavior
    Given I go to "http://localhost:3000/users"
